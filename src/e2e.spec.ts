...
import { tokensToCss } from "./index"
import type { DtcgNode } from "./types"

const here = __dirname
const fixturesDir = join(here, "fixtures")
const tokensFile = join(fixturesDir, "design-tokens.json")

describe("generateVariablesCss (e2e)", () => {
  it("produces scoped CSS with override-only contexts", async () => {
    const css = await generateVariablesCss({ cwd: fixturesDir, write: false })
    ...
    expect(css).toMatch(/\.dark \{[^}]*--foreground: oklch\(0\.93 0\.003 173\.9\);/)
    ...
    expect(css).toMatch(/\.dark \{[^}]*--primary-foreground: oklch\(0\.93 0\.003 173\.9\);/)
